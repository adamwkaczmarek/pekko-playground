...
/**
 * Entry point for the UserService cluster (backend).
 *
...
object UserServiceApp extends App {
  ...
  val configName = sys.env.getOrElse("CONFIG_RESOURCE", "user-service")
  ...
  implicit val system: ActorSystem[Nothing] = ActorSystem[Nothing](
    Behaviors.setup[Nothing] { ctx =>
      ...
      ctx.spawn[Nothing](ShardStatsPoller("User"), "shard-stats-poller")
  ...
  sharding.init(Entity(UserEntity.TypeKey)(ctx => UserEntity(ctx.entityId)))
  ...
  val port = config.getInt("user-service.http.port")
  val routes = concat(
    path("metrics") {
      get {
  ...
  system.log.info("UserService listening on port {} (internal, /metrics available)", port)

  sys.addShutdownHook {
    system.log.info("UserService shutting down...")
    system.terminate()
  }
